...
sf_zoning_codes = [
    ...
    'S-0.75P', 'SC-7', 'SC-10', 'T-6', 'T-5', 'SF', 'S10', 'S40', 'S25', 'S15',
    'S7', 'S4', 'R1'
]
...
brookline_milton_parcels['isZonedAsSF'] = brookline_milton_parcels[
    'ZONING'].apply(lambda zoning_code: zoning_code in sf_zoning_codes)
brookline_milton_parcels['isZonedAsMultifamily'] = brookline_milton_parcels[
    'ZONING'].apply(
        lambda zoning_code: zoning_code in multifamily_zoning_codes)
brookline_milton_parcels['isZonedAsCommercial'] = brookline_milton_parcels[
...
other_multi_family_use_codes = [
    string for string in [str(num) for num in range(106, 154 + 1)]
...
brookline_milton_parcels['isUsedAsCommercial'] = brookline_milton_parcels[
...
brookline_milton_parcels[
    'isUsedAsMultiBuildings1Lot'] = brookline_milton_parcels['USE_CODE'].apply(
...
brookline_milton_parcels[
    'isUsedAsOtherMultifamily'] = brookline_milton_parcels['USE_CODE'].apply(
        lambda use_code: use_code in other_multi_family_use_codes)

brookline_milton_parcels['mustUpzone'] = brookline_milton_parcels.apply(
...
brookline_milton_stop_zone_data = brookline_milton_parcels.groupby(
    'stop_name').agg({
        'municipality': 'first',
...
brookline_milton_stop_zone_data.rename(columns={
...
print(brookline_milton_stop_zone_data)
...
brookline_milton_stop_zone_census = brookline_milton_stop_zone_data.merge(
    station_buffer_census_cumulative, on="stop_name")
end_time = time.time()
elapsed_time = end_time - start_time

print(
    f"Time taken to compute brookline_milton_stop_zone_census {len(brookline_milton_stop_zone_census)} lines:",
    elapsed_time, "seconds")

brookline_milton_stop_zone_census.to_file(
    PREPROCESSED_DATA_PATH / "brookline_milton_stop_zone_census.geojson",
    driver='GeoJSON')
