<script>
    ...
    function drawCirclePacking() {
        ...
        function createLegend() {
            ...
            const categories = [
                {name: 'Future Commercial', color: '#97340b'},
                {name: 'Future Multi Family', color: '#999624'}
            ];
    ...
    async function dotInteraction(event, data) {
        ...
        if (event.type === "mouseenter" || event.type === "focus") {
            ...
            tooltipPosition = await computePosition(hoveredDot, treemapFutureTooltip, {
                strategy: "fixed",
                middleware: [
                    offset(5),
                    autoPlacement()
...
<dl class="info"
    ...
    bind:this={treemapFutureTooltip}
    role="chart-tooltip">
