...
<div class="slide">
    ...
    {:else}
        <h1>Let's consider {firstStation?.Name} and {secondStation?.Name} in {municipality?.Name}</h1>
         <h3> Now we compare their zoning, usage, and demographics. The effects of creating a multifamily district near each zone may be disparate or similar. Use this information to determine where a multi-family district siting would be most reasonable for your selected community.  </h3> <br>
            ...
            <table>
             ...
             <h3>
            In the area surrounding {firstStation?.Name}, there are currently {String(firstStationData?.pctZonedAsSF).slice(0,5)}% parcels zoned for single family residency, of which {String(firstStationData?.pctUsedAsSF).slice(0,5)}% are actually used as Single Family. This means that {String(firstStationData?.pctMustUpzone).slice(0,5)}% parcels (colored in orange) must upzone in order to comply with the MBTA communities Act, and {String(firstStationData?.pctWillChange).slice(0,5)}% will likely actually change their use.  </h3>
            <br>
            <br>
             <h3>
            In comparison, in the area surrounding {secondStation?.Name}, there are currently {String(secondStationData?.pctZonedAsSF).slice(0,5)}% parcels zoned for single-family residency, of which {String(secondStationData?.pctUsedAsSF).slice(0,5)}% as used as Single Family. This means that {String(secondStationData?.pctMustUpzone).slice(0,5)}% parcels (colored in orange) must upzone to comply with the MBTA communities Act, and {String(secondStationData?.pctWillChange).slice(0,5)}% will likely actually change their use.  </h3>
    {/if}
</div>
